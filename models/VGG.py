...
sys.path.append('/home/adi/Documents/Fine_grain/FG_classifier/base')
...
class VGG(BaseModel):
    def __init__(self, data_loader, config):
        ...
        self.images = None
        self.labels_fine = None
        self.labels_coarse = None
    ...
    def build_model(self):
        ...
        with tf.variable_scope('inputs'):
            self.images, self.labels_coarse = self.data_loader.get_input()
            self.is_training = tf.placeholder(tf.bool, name='Training_flag')
        tf.add_to_collection('inputs', self.images)
        tf.add_to_collection('inputs', self.labels_fine)
        tf.add_to_collection('inputs', self.labels_coarse)
        ...
        with tf.variable_scope('network'):
            conv1 = VGG.conv_bn_relu('conv1_block', self.images, 16, (3, 3), self.is_training)
            conv2 = VGG.conv_bn_relu('conv2_block', conv1, 32, (3, 3), self.is_training)

            with tf.variable_scope('max_pool1'):
                max_pool1 = tf.layers.max_pooling2d(conv2, pool_size=(2, 2), strides=(2, 2), name='max_pool')

            conv3 = VGG.conv_bn_relu('conv3_block', max_pool1, 32, (3, 3), self.is_training)
            conv4 = VGG.conv_bn_relu('conv4_block', conv3, 32, (3, 3), self.is_training)
            ...
            dense1 = VGG.dense_bn_relu_dropout('dense1', flattened, 512, 0.5, self.is_training)
            dense2 = VGG.dense_bn_relu_dropout('dense2', dense1, 256, 0.3, self.is_training)
        ...
        with tf.variable_scope('loss-acc'):
            self.loss = tf.losses.sparse_softmax_cross_entropy(labels=self.labels_coarse, logits=self.out)
            self.acc = tf.reduce_mean(tf.cast(tf.equal(self.labels_coarse, self.out_argmax), tf.float32))
